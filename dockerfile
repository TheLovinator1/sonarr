...
RUN addgroup -g 1000 sonarr && \
    ...
    install -d -o sonarr -g sonarr -m 775 /var/log/sonarr /var/lib/sonarr /usr/lib/sonarr/bin /sonarr_temp /downloads
...
ADD package_info /sonarr_temp

WORKDIR /sonarr_temp

RUN wget "${source}" && \ 
    tar -xf "Sonarr.develop.${pkgver}.linux.tar.gz" -C /sonarr_temp && \
